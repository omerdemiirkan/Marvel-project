...
export class SubmitOrderComponent implements OnInit {
  ...
  submitOrder() {
    const newCustomer: CustOrder = {
      ...
      prodOfferLists: this.store.selectSnapshot(AuthState.addOfferToBasket)
        .selectedOfferList,
      orderCharList: {
        charId: Math.floor(100 + Math.random() * 900),
        charValId: Math.floor(100 + Math.random() * 900),
        name: '',
        shortCode: '',
        value: '',
      },
    ...
    this.orderService.addOrder(newCustomer)?.subscribe(() => {
